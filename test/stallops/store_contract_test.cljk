(ns stallops.store-contract-test
  "Contract tests for `stallops.store/Store` protocol."
...
(deftest mem-store-stall-lookup
  (testing "MemStore can store and retrieve stalls by ID (string keys)"
    (let [stalls {"s1" {:stall-id "s1" :name "Alice's Textile Stall" :registered? true :verified? true}}
...
(deftest mem-store-all-stall-records
  (testing "MemStore returns all stalls in sorted order"
    (let [stalls {"s2" {:stall-id "s2" :name "Bob's Footwear Pitch"}
                  "s1" {:stall-id "s1" :name "Alice's Textile Stall"}
                  "s3" {:stall-id "s3" :name "Carol's Clothing Market Booth"}}
          s (store/mem-store stalls)
...
(deftest mem-store-vendor-lookup
  (testing "MemStore can store and retrieve vendors by ID (string keys)"
    (let [vendors {"v1" {:vendor-id "v1" :name "Acme Textile Manufacturing" :registered? true :verified? true}}
...
(deftest mem-store-all-vendor-records
  (testing "MemStore returns all vendors in sorted order"
    (let [vendors {"v2" {:vendor-id "v2" :name "Beta Footwear Supply"}
                   "v1" {:vendor-id "v1" :name "Acme Textile Manufacturing"}}
          s (store/mem-store {} vendors)
...
(deftest mem-store-coordination-log
  (testing "MemStore commit-record! appends to coordination-log"
    (let [s (store/mem-store {})
          record {:op :log-sales-record :stall-id "s1" :value {:units-sold 22}}]
...
(deftest mem-store-with-stall-records
  (testing "MemStore with-stall-records replaces the stall directory"
    (let [s (store/mem-store {})
          new-stalls {"s1" {:stall-id "s1" :name "Alice's Textile Stall"}}]
...
(deftest mem-store-with-vendor-records
  (testing "MemStore with-vendor-records replaces the vendor directory"
    (let [s (store/mem-store {})
          new-vendors {"v1" {:vendor-id "v1" :name "Acme Textile Manufacturing"}}]
...
(deftest seed-db-has-demo-data
  (testing "seed-db creates a populated MemStore with demo stalls and vendors"
    (let [s (store/seed-db)]
...
(deftest demo-data-string-key-consistency
  (testing "demo-data uses string keys, not keywords, for stall-id/vendor-id"
