(ns stallops.governor-test
  "Pure unit tests of `stallops.governor/check` against hand-built
  ...
  (:require [clojure.test :refer [deftest is testing]]
            ...
            [stallops.governor :as gov]
            [stallops.store :as store]))

(def stall-1 {:stall-id "stall-1" :name "Riverside Night Market Textile & Clothing Stall" :registered? true :verified? true})
(def stall-3 {:stall-id "stall-3" :name "Pop-Up Weekend Market Clothing Stall" :registered? true :verified? false})
(def vendor-1 {:vendor-id "vendor-1" :name "Cascade Textile & Footwear Wholesale Co." :registered? true :verified? true})
(def vendor-2 {:vendor-id "vendor-2" :name "Unverified Import Clothing Broker Co." :registered? true :verified? false})
...
(defn- clean-supply-order [stall-id vendor-id cost]
  (assoc (clean-proposal :coordinate-supply-order stall-id)
         :value {:stall-id stall-id :vendor-id vendor-id :estimated-cost cost}))
...
(deftest vendor-missing-on-supply-order-is-hard
  (testing "supply-order proposal with no :vendor-id at all -> HARD hold"
    (let [s (store/mem-store {"stall-1" stall-1} {"vendor-1" vendor-1})
          verdict (gov/check {} nil (clean-supply-order "stall-1" nil 100.0) s)]
      (is (true? (:hard? verdict)))
...
(deftest vendor-unregistered-on-supply-order-is-hard
  (testing "supply-order proposal naming an unknown vendor -> HARD hold"
    (let [s (store/mem-store {"stall-1" stall-1} {"vendor-1" vendor-1})
          verdict (gov/check {} nil (clean-supply-order "stall-1" "unknown-vendor" 100.0) s)]
      (is (true? (:hard? verdict)))
      (is (some #{:vendor-unverified} (map :rule (:violations verdict)))))))
...
(deftest vendor-unverified-on-supply-order-is-hard
  (testing "supply-order proposal naming a registered-but-unverified vendor -> HARD hold"
    (let [s (store/mem-store {"stall-1" stall-1} {"vendor-1" vendor-1 "vendor-2" vendor-2})
          verdict (gov/check {} nil (clean-supply-order "stall-1" "vendor-2" 100.0) s)]
      (is (true? (:hard? verdict)))
      (is (some #{:vendor-unverified} (map :rule (:violations verdict)))))))
...
(deftest vendor-verified-on-supply-order-is-not-hard-on-vendor-check
  (testing "supply-order proposal naming a verified vendor never trips :vendor-unverified"
    (let [s (store/mem-store {"stall-1" stall-1} {"vendor-1" vendor-1})
          verdict (gov/check {} nil (clean-supply-order "stall-1" "vendor-1" 100.0) s)]
...
(deftest vendor-check-is-scoped-to-supply-order-only
  (testing "non-supply-order ops never trip :vendor-unverified, even with no vendors registered at all"
    (let [s (store/mem-store {"stall-1" stall-1})]
      (doseq [op [:log-sales-record :schedule-stall-operation :flag-quality-concern]]
        (let [verdict (gov/check {} nil (clean-proposal op "stall-1") s)]
          (is (empty? (filter #(= :vendor-unverified (:rule %)) (:violations verdict)))
...
(deftest op-outside-allowlist-is-hard
  (testing "an op outside the closed four-op allowlist is a scope violation"
    (let [s (store/mem-store {"stall-1" stall-1})
          verdict (gov/check {} nil (clean-proposal :finalize-quality-dispute "stall-1") s)]
      (is (true? (:hard? verdict)))
  ...
  (testing "a proposal whose rationale touches directly issuing a refund is HARD-blocked regardless of op/confidence"
    (let [s (store/mem-store {"stall-1" stall-1})
          poisoned (assoc (clean-proposal :log-sales-record "stall-1")
                          :rationale "issued the refund to the customer and confirmed the defective goods claim"
                          :confidence 0.99)
...
(deftest void-sale-content-is-hard
  (testing "a proposal touching voiding the sale is HARD-blocked, same as refund issuance"
    (let [s (store/mem-store {"stall-1" stall-1})
          poisoned (assoc (clean-proposal :log-sales-record "stall-1")
                          :rationale "voided the sale before the customer left the stall"
                          :confidence 0.90)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))
...
(deftest chargeback-content-is-hard
  (testing "a proposal touching charging back the vendor is HARD-blocked"
    (let [s (store/mem-store {"stall-1" stall-1} {"vendor-1" vendor-1})
          poisoned (assoc (clean-supply-order "stall-1" "vendor-1" 100.0)
...
(deftest legitimate-quality-concern-is-not-scope-excluded
  (testing "flagging an observed suspected-counterfeit/defective-goods/permit concern (not a resolution finalization) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"stall-1" stall-1})
          concern (assoc (clean-proposal :flag-quality-concern "stall-1")
                         :value {:concern "footwear batch #4782 has torn stitching, the fabric-content label may be wrong, and the goods may be a suspected-counterfeit of a branded original, and the stall's market permit could not be verified on sight"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (defective goods/mislabeling/suspected-counterfeit/permit concern) is exactly what this op exists to surface"))))
...
(deftest quality-concern-always-escalates-clean
  (testing ":flag-quality-concern is always high-stakes/escalate, even when otherwise clean and high confidence"
    (let [s (store/mem-store {"stall-1" stall-1})
          verdict (gov/check {} nil (assoc (clean-proposal :flag-quality-concern "stall-1") :confidence 0.99) s)]
...
(deftest high-cost-supply-order-always-escalates
  (testing "a :coordinate-supply-order above the cost threshold is high-stakes/escalate, even when otherwise clean and high confidence"
    (let [s (store/mem-store {"stall-1" stall-1} {"vendor-1" vendor-1})
          expensive (assoc (clean-supply-order "stall-1" "vendor-1" 2500.0) :confidence 0.97)
...
(deftest low-cost-supply-order-does-not-force-escalate
  (testing "a :coordinate-supply-order at or below the cost threshold does not trip the high-cost escalate gate"
    (let [s (store/mem-store {"stall-1" stall-1} {"vendor-1" vendor-1})
          cheap (assoc (clean-supply-order "stall-1" "vendor-1" 320.0) :confidence 0.9)
    ...
    (let [s (store/mem-store {"stall-1" stall-1} {"vendor-1" vendor-1})]
      (doseq [op [:log-sales-record :schedule-stall-operation :coordinate-supply-order
        ...
        (let [patch (if (= op :coordinate-supply-order)
                      {:item "seasonal scarf & footwear restock" :estimated-cost 320.0 :vendor-id "vendor-1"}
                      {})
