(ns stallops.governor-contract-test
  ...
  (testing "clean sales-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-sales-record :stall-id "stall-1" :patch {:units-sold 22}}
                               ctx)]
      (is (some? result))
  ...
  (testing ":flag-quality-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-quality-concern :stall-id "stall-1"
                                :patch {:concern "suspected-counterfeit footwear batch #4782" :confidence 0.99}}
                               ctx)]
      (is (some? result))
  ...
  (testing "a high-cost :coordinate-supply-order escalates for human approval, even at phase 3 clean"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :stall-id "stall-1"
                                :patch {:item "bulk woven-textile bolt order" :quantity 1 :estimated-cost 1800.0
                                        :vendor-id "vendor-1"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest unregistered-stall-hard-hold
  (testing "unregistered stall -> permanent HARD hold, never escalates"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-3" :phase 3}]
      (exec-request actor "t3"
                     {:op :log-sales-record :stall-id "unknown-stall"
                      :patch {:units-sold 0}}
                     ctx)
      (is (= 0 (count (store/coordination-log db)))
          "HARD hold must never commit"))))

(deftest unverified-stall-hard-hold
  (testing "registered but unverified stall -> permanent HARD hold"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-4" :phase 3}
          result (exec-request actor "t4"
                               {:op :log-sales-record :stall-id "stall-3"
                                :patch {:units-sold 5}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
          "unverified stall must HARD hold"))))
  ...
  (testing "proposal with :effect :commit (not :propose) -> hard hold"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t5"
                               {:op :log-sales-record :stall-id "stall-1"
                                :patch {:units-sold 22}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into quality-dispute-resolution/counterfeit-determination-finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8b"
                     {:op :log-sales-record :stall-id "unknown" :patch {:units-sold 22}}
                     ctx)
