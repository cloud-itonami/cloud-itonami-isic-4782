(ns stallops.phase-test
  "Unit tests of `stallops.phase` rollout logic."
...
(deftest phase-1-sales-record-only
  (testing "phase 1 allows only sales-record logging, requires approval"
    ...
    (let [{:keys [disposition]} (phase/gate 1 {:op :schedule-stall-operation} :commit)]
      (is (= :hold disposition)))))
...
(deftest phase-3-auto-commits-clean-ops
  (testing "phase 3 auto-commits clean, high-conf non-safety ops"
    ...
    (let [{:keys [disposition]} (phase/gate 3 {:op :schedule-stall-operation} :commit)]
      (is (= :commit disposition)))
