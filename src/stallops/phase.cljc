(ns stallops.phase
  ...
  rollout milestone still to come. Flagging a suspected-counterfeit/
  defective-goods/stall-permit concern always needs a human to actually
  look at it. `stallops.governor`'s own `always-escalate-ops` enforces
  the same invariant independently -- two layers, not one, agree on
  this."
  (:require [stallops.governor :as governor]))

(def read-ops #{})
(def write-ops governor/allowed-ops)

;; NOTE the invariant: `:flag-quality-concern` is a member of `write-ops`
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                              :auto #{}}
   ...
   2 {:label "assisted-coordination"  :writes #{:log-sales-record :schedule-stall-operation
                                                 :coordinate-supply-order}                                     :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-sales-record :schedule-stall-operation :coordinate-supply-order}}})
...
(defn verdict->disposition
  "Map a StallMarketGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
