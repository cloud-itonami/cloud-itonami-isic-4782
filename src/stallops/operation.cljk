(ns stallops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (StallMarketAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the StallMarketGovernor (:govern)
  ...
  human market-stall coordinator. The approver resumes with `{:approval
  {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [stallops.advisor :as advisor]
            [stallops.governor :as governor]
            [stallops.phase :as phase]
...
(defn build
  ...
  `stallops.store/Store`).
  opts:
    :advisor      -- a `stallops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; StallMarketAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; StallMarketGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :stall-id (:stall-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :stall-id (:stall-id request) :by (:by approval)}]}
            {:disposition :hold
